...
const CENTS = [
  //  A,     C,     D,     E,     G
      0,   300,   500,   700,  1000,
   1200,  1500,  1700,  1900,  2200,
   2400,  2700,  2900,  3100,  3400,
   3600,  3900,  4100,  4300,  4600,
   4800,  5100,  5300,  5500,  5800,
   6000,  6300,  6500,  6700,  7000,
   7200,  7500,  7700,  7900,  8200,
   8400,  8700,  8900,  9100,  9400,
   9600,  9900, 10100, 10300, 10600,
  10800, 11100, 11300, 11500, 11800,
]
  ...
  beat() {
    ...
    if (cursor === 0) {
      ...
      for (let y = 0; y < height; ++y) {
        ...
        for (let x = 0; x < width; ++x) {
          let neighbor = 0;
          if (field[(y + height - 1) % height][(x + width - 1) % width])
            ++neighbor;
          ...
          nextRow.push(
            (field[y][x] && (neighbor === 2 || neighbor === 3)) ||
              (!field[y][x] && neighbor === 3)
          );
        }
        ...
        <div id="controls">
          ...
          <label>
            ...
            <input
              type="number"
              value={bpm}
              onChange={ev => this.handlebpmChange(ev)}
            />
          </label>
        </div>
      </div>
